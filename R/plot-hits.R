...
plot.bagel2 <- function(a,b,c){
  
  work.dir <- a
  df <- read.csv(file=b, sep="\t")
  save.path <- args[3]
  title <- args[4]
  ...
  file.plot <- paste0(work.dir,"bagel2","/precision-recall-",title, ".pdf")
...
} else if(test == "bagel2"){plot.bagel2(work.dir,df.file,save.path)
    }